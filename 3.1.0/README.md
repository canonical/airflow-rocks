...
This document explains how to build, customize, and work with the **Apache Airflow Rock** defined in `rockcraft.yaml`.
...
Once built, you can run the rock locally using `rockcraft` or `docker`:

### Using Rockcraft

```bash
rockcraft.skopeo --insecure-policy copy oci-archive:airflow-rock_3.1.0_amd64.rock docker-daemon:airflow-rock:3.1.0
docker run -it --rm -p 8080:8080 airflow-rock:3.1.0
```
